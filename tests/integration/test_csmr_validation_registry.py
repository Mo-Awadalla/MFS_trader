"""Integration tests for CSMR registry, artifacts, and validation pipeline."""
...
from research.csmr_pipeline import backtest_csmr, run_csmr_validation_gauntlet
from strategies.csmr.signal import CSMRParams


def csmr_panel(n_days: int = 620, n_symbols: int = 120) -> pd.DataFrame:
    ...
    for i, symbol in enumerate(symbols):
        base = 30.0 + i
        noise = np.sin(np.arange(n_days) / 9.0 + i) * 0.002
        trend = 1.0 + np.cumsum(noise)
        close = base * trend
...
def test_csmr_backtest_is_deterministic() -> None:
    df = csmr_panel()
    params = CSMRParams()

    first = backtest_csmr(df, params)
    second = backtest_csmr(df, params)
...
def test_csmr_validation_registers_experiment_and_writes_artifacts(tmp_path: Path) -> None:
    df = csmr_panel()
    registry = ExperimentRegistry(tmp_path / "experiments")
    artifacts = ArtifactManager(registry.root)
    try:
        report = run_csmr_validation_gauntlet(
            df,
            registry=registry,
            artifacts=artifacts,
            experiment_label="CSMR-v1-synthetic-validation",
        ...
        assert experiment.snapshot.strategy == "cross_sectional_mean_reversion"
        assert experiment.snapshot.parameters["lookback_days"] == 5
        ...
        assert validation_report["strategy"] == "cross_sectional_mean_reversion"
