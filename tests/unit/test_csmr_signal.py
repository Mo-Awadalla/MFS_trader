"""Tests for canonical CSMR v1 signal construction."""
...
class TestCSMRSignal:
    def test_rebalance_forms_dollar_neutral_top_bottom_quartiles(self) -> None:
        ...
        assert "S000" in long_symbols
        assert "S119" in short_symbols
    ...
    def test_uses_prior_data_only(self) -> None:
        ...
        pd.testing.assert_frame_equal(
            original.loc[: "2024-04-01"],
            changed.loc[: "2024-04-01"],
        )

    def test_skips_rebalance_below_minimum_universe_and_retains_holdings(self) -> None:
        df = _csmr_frame(n_symbols=120)
        symbols_to_break = [f"S{i:03d}" for i in range(80)]
        df.loc["2024-04-02":, [(symbol, "close") for symbol in symbols_to_break]] = np.nan
        ...
        assert bool(skipped.loc["2024-04-08"])
        assert reason.loc["2024-04-08"] == "insufficient_eligible_universe"
        pd.testing.assert_series_equal(
            weights.loc["2024-04-08"],
    ...
    def test_validates_multi_symbol_ohlcv_shape(self) -> None:
        ...
        with pytest.raises(ValueError, match="MultiIndex"):
            generate_signals(bad, CSMRParams())
    ...
    def test_requires_complete_ohlcv_fields_for_every_symbol(self) -> None:
        ...
        with pytest.raises(ValueError, match="missing fields"):
            generate_signals(incomplete, CSMRParams())
