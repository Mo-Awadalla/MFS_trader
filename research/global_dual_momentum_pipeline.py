"""Global Dual Momentum Defensive research + validation pipeline."""
...
from research.global_dual_momentum_experiment import (
    STRATEGY_NAME,
    build_global_dual_momentum_experiment_draft,
)
...
GlobalDualMomentumBacktestResult = CrossSectionalBacktestResult
GlobalDualMomentumValidationReport = CrossSectionalValidationReport


def backtest_global_dual_momentum(
    df: pd.DataFrame,
    ...
    *,
    ...
    """Run a vectorized Global Dual Momentum backtest."""
...
def run_global_dual_momentum_validation_gauntlet(
    df: pd.DataFrame,
    *,
    params: GlobalDualMomentumParams | None = None,
    cost_config: CostModelConfig | None = None,
    ...
    """Run Global Dual Momentum research and full Validation Gauntlet."""
    ...
    research = backtest_global_dual_momentum(
        df,
        params,
        cost_config=cost_config,
        initial_capital=initial_capital,
    ...
    train_fn, test_fn = make_global_dual_momentum_wfa_fns(
        df,
        params=params,
        cost_config=cost_config,
        initial_capital=initial_capital,
    ...
    replay_attribution = {
        ...
        "reason": "engine replay is currently single-symbol; Global Dual Momentum evidence uses deterministic vectorized ETF target weights",
    ...
    report = CrossSectionalValidationReport(
        strategy_name=STRATEGY_NAME,
        report_title="Global Dual Momentum Defensive v1 Validation Report",
        params=params,
...
def format_global_dual_momentum_gauntlet_report(report: CrossSectionalValidationReport) -> str:
