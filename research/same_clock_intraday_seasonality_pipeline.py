"""SameClockIntradaySeasonalityETF-v1 research + validation pipeline."""
...
from research.same_clock_intraday_seasonality_experiment import (
    STRATEGY_NAME,
    build_same_clock_intraday_seasonality_experiment_draft,
    default_same_clock_intraday_seasonality_cost_config,
)
...
PARAM_COLUMNS = [
    "lookback_sessions",
    "min_same_clock_mean_return",
    "skip_opening_bars",
    "exit_before_close_bars",
    "rebalance_frequency",
]
INTRADAY_WFA_CONFIG = WFAConfig(WFATier.PRIMARY, "180d", "60d", "30d")
SameClockIntradaySeasonalityBacktestResult = CrossSectionalBacktestResult


@dataclass
class SameClockIntradaySeasonalityValidationReport(CrossSectionalValidationReport):
...
def backtest_same_clock_intraday_seasonality(
    df: pd.DataFrame,
    ...
    *,
    ...
    """Run vectorized 30-minute ETF same-clock intraday seasonality backtest."""
...
def run_same_clock_intraday_seasonality_sweep(
    df: pd.DataFrame,
    *,
    params: SameClockIntradaySeasonalityParams | None = None,
    cost_config: CostModelConfig | None = None,
...
def make_same_clock_intraday_seasonality_wfa_fns(
    df: pd.DataFrame,
    *,
    params: SameClockIntradaySeasonalityParams | None = None,
    cost_config: CostModelConfig | None = None,
    ...
    params = params or default_params()
    cost_config = cost_config or default_same_clock_intraday_seasonality_cost_config()
    full_result = backtest_same_clock_intraday_seasonality(
        df,
        params,
    ...
    *,
...
) -> SameClockIntradaySeasonalityValidationReport:
    """Run same-clock research and full gauntlet plus intraday gates."""
    params = params or default_params()
    cost_config = cost_config or default_same_clock_intraday_seasonality_cost_config()
    ...
    if registry is not None:
        if active_experiment is None:
            ...
            draft = build_same_clock_intraday_seasonality_experiment_draft(
                df,
    ...
    research = backtest_same_clock_intraday_seasonality(
        df,
        params,
        cost_config=cost_config,
        initial_capital=initial_capital,
    )
    sweep_df = run_same_clock_intraday_seasonality_sweep(
        df,
        params=params,
        cost_config=cost_config,
        initial_capital=initial_capital,
    )
    train_fn, test_fn = make_same_clock_intraday_seasonality_wfa_fns(
        df,
        params=params,
        cost_config=cost_config,
        initial_capital=initial_capital,
    ...
    report = SameClockIntradaySeasonalityValidationReport(
        strategy_name=STRATEGY_NAME,
        report_title="Same-Clock Intraday Seasonality ETF v1 Validation Report",
        params=params,
        ...
        replay_attribution={
            ...
            "execution_alignment": "same-clock target is written one bar before the traded slot; zero target before close prevents overnight exposure",
        },
    ...
    if write_artifacts and artifacts is not None and active_experiment is not None:
        write_same_clock_intraday_seasonality_validation_artifacts(artifacts, active_experiment.uuid, report)
    return report


def format_same_clock_intraday_seasonality_gauntlet_report(
    report: SameClockIntradaySeasonalityValidationReport,
) -> str:
...
def write_same_clock_intraday_seasonality_validation_artifacts(
    ...
    artifacts.write_text(
        ...
        format_same_clock_intraday_seasonality_gauntlet_report(report) + "\n",
    )
    artifacts.write_text(
        experiment_uuid,
