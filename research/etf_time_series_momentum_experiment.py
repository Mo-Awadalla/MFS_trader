"""ETF time-series momentum experiment draft builder."""
...
STRATEGY_NAME = "etf_time_series_momentum"
...
def build_etf_time_series_momentum_experiment_draft(
    df: pd.DataFrame,
    *,
    label: str,
    ...
    """Build a registry-ready draft for ETF time-series momentum v1."""
    ...
    portfolio_config = portfolio_config or PortfolioConfig(
        sizing_method="inverse_volatility_vol_target",
    ...
    if data_version is None:
        data_version = f"{data_source}:etf_tsm_static_v1:{bar_frequency}@{len(df)}x{len(symbols)}"
    ...
    snapshot = ExperimentSnapshot(
        ...
        universe=UniverseSpec(
            ...
            selection_rule="static_source_declared_etf_time_series_momentum_v1",
            filters={
                ...
                "selection_rule": "top_n_positive_absolute_momentum_else_fallback_symbol",
                "top_n": params.top_n,
                "fallback_symbol": params.fallback_symbol,
                "sizing_rule": "inverse_volatility_then_realized_volatility_target",
