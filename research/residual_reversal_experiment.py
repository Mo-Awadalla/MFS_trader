"""ResidualReversalStatArb experiment draft builder."""
...
STRATEGY_NAME = "residual_reversal_stat_arb"
...
def build_residual_reversal_experiment_draft(
    df: pd.DataFrame,
    *,
    label: str,
    ...
    """Build a registry-ready draft for frozen ResidualReversalStatArb v1."""
    params = params or default_params()
    cost_config = cost_config or default_cost_config()
    risk_profile = risk_profile or RiskLimits(
        max_gross_exposure_pct=2.0,
        max_net_exposure_pct=0.0,
        max_open_positions=max(params.selection_count * 2, params.min_eligible_symbols),
    )
    ...
    if data_version is None:
        data_version = f"{data_source}:residual_filtered_us_equities:{bar_frequency}@{len(df)}x{len(symbols)}"
    ...
    snapshot = ExperimentSnapshot(
        ...
        universe=UniverseSpec(
            symbols=symbols,
            asset_class="equity",
            selection_rule="daily_tradability_filters_excluding_factor_etfs",
            filters={
