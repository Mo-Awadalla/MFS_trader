"""Market intraday momentum experiment draft builder."""
...
STRATEGY_NAME = "market_intraday_momentum"
...
def default_market_intraday_momentum_cost_config() -> CostModelConfig:
...
def _slippage_from_cost(cost: CostModelConfig) -> dict[str, Any]:
    return {
        ...
        "bar_execution_model": "signal_at_bar_close_execute_next_30m_bar_with_conservative_slippage",
...
def build_market_intraday_momentum_experiment_draft(
    df: pd.DataFrame,
    *,
    label: str,
    ...
    storage_dir: str = "data/parquet/market_intraday_momentum/alpaca_sip",
    ...
    """Build a registry-ready draft for market intraday momentum v1."""
    ...
    if data_version is None:
        data_version = f"{data_source}:market_intraday_momentum_v1:{bar_frequency}@{len(df)}x{len(symbols)}"
    ...
    snapshot = ExperimentSnapshot(
        ...
        universe=UniverseSpec(
            ...
            selection_rule="static_liquid_index_etfs_market_intraday_momentum_v1",
            filters={
        ...
        execution_mode="first_30m_signal_execute_next_bar_exit_before_close_no_overnight",
