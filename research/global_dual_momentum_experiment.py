"""Global Dual Momentum Defensive experiment draft builder."""
...
STRATEGY_NAME = "global_dual_momentum"
...
def build_global_dual_momentum_experiment_draft(
    df: pd.DataFrame,
    *,
    label: str,
    ...
    """Build a registry-ready draft for Global Dual Momentum Defensive v1."""

    params = params or default_params()
    cost_config = cost_config or default_cost_config()
    risk_profile = risk_profile or RiskLimits(
        max_gross_exposure_pct=params.target_gross,
        max_net_exposure_pct=params.target_gross,
        max_open_positions=max(params.top_risk_n, params.defensive_n),
    )
    ...
    if data_version is None:
        data_version = f"{data_source}:global_dual_momentum_static_v1:{bar_frequency}@{len(df)}x{len(symbols)}"
    ...
    snapshot = ExperimentSnapshot(
        ...
        universe=UniverseSpec(
            ...
            selection_rule="static_source_declared_global_dual_momentum_v1",
            filters={
                ...
                "absolute_momentum_lookback_days": params.momentum_lookback_days,
                "risk_on_rule": "hold_top_risk_asset_only_if_positive_absolute_momentum",
                "risk_off_rule": "hold_best_defensive_asset_when_no_positive_risk_asset",
                "top_risk_n": params.top_risk_n,
                "defensive_n": params.defensive_n,
                "source_memo": "docs/strategy_sources/GlobalDualMomentumDefensive-v1.md",
            },
        ),
