...
def main():
    ...
    for day, bars in sessions:
        ...
        morning_center = center(bars.iloc[0:120])
        lunch_center = center(bars.iloc[120:240])
        lunch_close = float(bars.iloc[239].close)
        if (morning_center - cash_open) * (lunch_center - cash_open) >= 0:
            no_transfer += 1
            continue
        if lunch_center > cash_open and lunch_close > cash_open:
            side = 1
        elif lunch_center < cash_open and lunch_close < cash_open:
            side = -1
        else:
            no_confirmation += 1
            continue
    ...
    output = {
        ...
        "no_polarity_transfer": no_transfer,
        "no_terminal_confirmation": no_confirmation,
