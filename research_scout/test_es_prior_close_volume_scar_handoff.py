import json
import math
import sys
from pathlib import Path

sys.path.insert(0, ".vendor")

import databento as db
import numpy as np
import pandas as pd

ROOT = Path("external_artifacts/databento_es_close/GLBX.MDP3")
BAR_PATH = ROOT / "bars" / "es_ohlcv_1m_2021-01-01_2026-01-01.dbn.zst"
SESSION_ROOT = ROOT / "sessions"
TICK_SIZE = 0.25
POINT_VALUE = 5.0
COMMISSION = 1.24


def load_sessions():
    frame = db.DBNStore.from_file(BAR_PATH).to_df()
    if frame.index.tz is None:
        frame.index = frame.index.tz_localize("UTC")
    frame = frame.tz_convert("America/New_York")
    rth = frame.between_time("09:30", "15:59").copy()
    rth["day"] = rth.index.date
    sessions = []
    for day, bars in rth.groupby("day"):
        if len(bars) >= 390:
            sessions.append((pd.Timestamp(day), bars.drop(columns="day").iloc[:390]))
    return sessions


def closing_scar(day):
    path = (
        SESSION_ROOT
        / str(day.year)
        / day.strftime("%Y-%m-%d")
        / "es_trades.dbn.zst"
    )
    if not path.exists():
        return None
    trades = db.DBNStore.from_file(path).to_df()
    if trades.empty or trades.instrument_id.nunique() != 1:
        return None
    by_price = trades.groupby("price", sort=True)["size"].sum()
    maximum = by_price.max()
    candidates = by_price[by_price == maximum].index.to_numpy(dtype=float)
    vwap = float(np.average(trades.price, weights=trades["size"]))
    distances = np.abs(candidates - vwap)
    scar = float(candidates[distances == distances.min()].min())
    return scar, int(trades.instrument_id.iloc[0])


def center(bars):
    typical = (bars.high + bars.low + bars.close) / 3.0
    return float((typical * bars.volume).sum() / bars.volume.sum())


def profit_factor(values):
    gains = float(values[values > 0].sum())
    losses = float(-values[values < 0].sum())
    return gains / losses if losses else math.inf


def build_and_simulate(sessions):
    scar_by_day = {day: closing_scar(day) for day, _ in sessions}
    records = []
    exclusions = {}
    for year in {2021, 2022, 2023, 2024, 2025}:
        exclusions[year] = {
            "missing_prior_scar": 0,
            "roll_mismatch": 0,
            "no_signal": 0,
            "bad_stop_geometry": 0,
        }
    for index in range(1, len(sessions)):
        day, bars = sessions[index]
        prior_day = sessions[index - 1][0]
        prior = scar_by_day.get(prior_day)
        if prior is None:
            exclusions[day.year]["missing_prior_scar"] += 1
            continue
        scar, prior_instrument = prior
        current_ids = pd.unique(bars.iloc[:30].instrument_id)
        if len(current_ids) != 1 or int(current_ids[0]) != prior_instrument:
            exclusions[day.year]["roll_mismatch"] += 1
            continue
        opening = bars.iloc[:30]
        opening_center = center(opening)
        opening_close = float(opening.iloc[-1].close)
        if opening_center > scar and opening_close > scar:
            side = 1
        elif opening_center < scar and opening_close < scar:
            side = -1
        else:
            exclusions[day.year]["no_signal"] += 1
            continue

        entry_idx = 30
        exit_idx = 360
        entry = float(bars.iloc[entry_idx].open) + side * TICK_SIZE
        stop = opening_center - side * TICK_SIZE
        if (side == 1 and stop >= entry) or (side == -1 and stop <= entry):
            exclusions[day.year]["bad_stop_geometry"] += 1
            continue

        lows = bars.low.to_numpy(dtype=float)
        highs = bars.high.to_numpy(dtype=float)
        stop_hits = (
            np.flatnonzero(lows[entry_idx:exit_idx] <= stop)
            if side == 1
            else np.flatnonzero(highs[entry_idx:exit_idx] >= stop)
        )
        if len(stop_hits):
            actual_exit_idx = entry_idx + int(stop_hits[0])
            bar_open = float(bars.iloc[actual_exit_idx].open)
            opened_beyond = bar_open < stop if side == 1 else bar_open > stop
            exit_price = (
                bar_open - side * TICK_SIZE if opened_beyond else stop
            )
            reason = "stop"
        else:
            exit_price = float(bars.iloc[exit_idx].open) - side * TICK_SIZE
            reason = "1530"

        pnl = side * (exit_price - entry) * POINT_VALUE - COMMISSION
        records.append(
            {
                "date": day,
                "side": side,
                "pnl": pnl,
                "reason": reason,
                "risk_usd": abs(entry - stop) * POINT_VALUE + COMMISSION,
            }
        )
    return pd.DataFrame(records), exclusions


def summarize(trades, dates):
    daily = trades.groupby("date").pnl.sum().reindex(dates, fill_value=0.0)
    drawdown = daily.cumsum() - daily.cumsum().cummax()
    yearly = trades.groupby(trades.date.dt.year).pnl.sum()
    sides = trades.groupby("side").pnl.sum()
    reasons = trades.groupby("reason").pnl.agg(["count", "sum"])
    years = int(trades.date.dt.year.nunique())
    return {
        "trades": int(len(trades)),
        "trades_per_week": round(len(trades) / (years * 52), 3),
        "net_pnl_usd": round(float(trades.pnl.sum()), 2),
        "mean_trade_usd": round(float(trades.pnl.mean()), 3),
        "profit_factor": round(profit_factor(trades.pnl), 3),
        "win_rate": round(float((trades.pnl > 0).mean()), 3),
        "max_daily_drawdown_usd": round(float(drawdown.min()), 2),
        "median_initial_risk_usd": round(float(trades.risk_usd.median()), 2),
        "year_pnl_usd": {
            str(int(year)): round(float(value), 2)
            for year, value in yearly.items()
        },
        "side_pnl_usd": {
            "long": round(float(sides.get(1, 0.0)), 2),
            "short": round(float(sides.get(-1, 0.0)), 2),
        },
        "exit_results": {
            str(reason): {
                "trades": int(values["count"]),
                "net_pnl_usd": round(float(values["sum"]), 2),
            }
            for reason, values in reasons.iterrows()
        },
    }


def rolling_diagnostics(trades, dates):
    daily = trades.groupby("date").pnl.sum().reindex(dates, fill_value=0.0)
    result = {}
    for window, label in [(63, "3m"), (126, "6m"), (252, "12m")]:
        values = daily.rolling(window).sum().dropna()
        if len(values):
            result[label] = {
                "minimum_usd": round(float(values.min()), 2),
                "median_usd": round(float(values.median()), 2),
                "maximum_usd": round(float(values.max()), 2),
                "fraction_positive": round(float((values > 0).mean()), 3),
            }
    return result


def block_bootstrap_loss_probability(trades, dates, seed=20260726):
    daily = (
        trades.groupby("date").pnl.sum().reindex(dates, fill_value=0.0)
    ).to_numpy(dtype=float)
    block = 5
    blocks = np.array(
        [daily[start : start + block] for start in range(len(daily) - block + 1)]
    )
    rng = np.random.default_rng(seed)
    draws = 10000
    block_count = math.ceil(len(daily) / block)
    totals = np.empty(draws)
    for draw in range(draws):
        chosen = blocks[rng.integers(0, len(blocks), size=block_count)].ravel()
        totals[draw] = chosen[: len(daily)].sum()
    return {
        "method": "10000 five-session block resamples",
        "seed": seed,
        "probability_total_pnl_le_zero": round(float((totals <= 0).mean()), 4),
        "p05_total_pnl_usd": round(float(np.quantile(totals, 0.05)), 2),
        "median_total_pnl_usd": round(float(np.median(totals)), 2),
    }


def stress_diagnostics(trades, dates):
    ordered = trades.sort_values("pnl", ascending=False)
    work = trades.copy()
    work["month"] = work.date.dt.to_period("M")
    work["year"] = work.date.dt.year
    best_months = (
        work.groupby(["year", "month"]).pnl.sum().groupby(level=0).idxmax()
    )
    best_month_values = [
        float(work[(work.year == year) & (work.month == month)].pnl.sum())
        for year, month in best_months
    ]
    return {
        "additional_1_25_usd_per_trade": round(
            float(trades.pnl.sum() - 1.25 * len(trades)), 2
        ),
        "additional_2_50_usd_per_trade": round(
            float(trades.pnl.sum() - 2.50 * len(trades)), 2
        ),
        "remove_five_best_trades": round(float(ordered.iloc[5:].pnl.sum()), 2),
        "remove_ten_best_trades": round(float(ordered.iloc[10:].pnl.sum()), 2),
        "five_best_trade_pnl_usd": [
            round(float(value), 2) for value in ordered.head(5).pnl
        ],
        "remove_each_year_best_month": round(
            float(trades.pnl.sum() - sum(best_month_values)), 2
        ),
        "best_month_pnl_by_year_usd": {
            str(year): round(value, 2)
            for (year, _), value in zip(best_months, best_month_values, strict=True)
        },
        "rolling": rolling_diagnostics(trades, dates),
        "five_day_block_bootstrap": block_bootstrap_loss_probability(
            trades, dates
        ),
    }


def period_exclusions(exclusions, years):
    keys = [
        "missing_prior_scar",
        "roll_mismatch",
        "no_signal",
        "bad_stop_geometry",
    ]
    return {
        key: int(sum(exclusions[year][key] for year in years)) for key in keys
    }


def main():
    sessions = load_sessions()
    trades, exclusions = build_and_simulate(sessions)
    all_dates = pd.DatetimeIndex([day for day, _ in sessions])
    development_years = {2021, 2022, 2023}
    holdout_years = {2024, 2025}
    development = trades[trades.date.dt.year.isin(development_years)].copy()
    holdout = trades[trades.date.dt.year.isin(holdout_years)].copy()
    development_dates = all_dates[all_dates.year.isin(development_years)]
    holdout_dates = all_dates[all_dates.year.isin(holdout_years)]
    output = {
        "development": {
            "period": "2021-2023",
            "sessions": int(len(development_dates)),
            "exclusions": period_exclusions(exclusions, development_years),
            "result": summarize(development, development_dates),
            "stress": stress_diagnostics(development, development_dates),
        },
        "secondary_holdout": {
            "period": "2024-2025",
            "sessions": int(len(holdout_dates)),
            "exclusions": period_exclusions(exclusions, holdout_years),
            "result": summarize(holdout, holdout_dates),
            "stress": stress_diagnostics(holdout, holdout_dates),
        },
    }
    print(json.dumps(output, indent=2, allow_nan=False))


if __name__ == "__main__":
    main()
