"""Momentum v1 — canonical cross-sectional 12-1 StrategyTemplate."""
...
class CrossSectionalMomentumStrategy(StrategyTemplate[MomentumParams]):
    """Dollar-neutral cross-sectional 12-1 momentum template."""

    name = "cross_sectional_momentum"
    template_version = TEMPLATE_VERSION

    def generate_signals(self, df: pd.DataFrame, params: MomentumParams | None = None) -> pd.DataFrame:
        self.validate_inputs(df)
        return generate_signals(df, params or self.default_params())

    def default_params(self) -> MomentumParams:
        return default_params()

    def sweep_grid(self) -> list[MomentumParams]:
        return sweep_grid()

    def compact_sweep_grid(self) -> list[MomentumParams]:
    ...
    def diagnose_signals(
        ...
        return StrategyDiagnostics(
            ...
            extra={
                ...
                "hypothesis": "recent_winners_outperform_recent_losers",
                "formation": "12-1",
    ...
    def warmup_bars(self, params: MomentumParams | None = None) -> int:
        params = params or self.default_params()
        return max(
    ...
    @staticmethod
    def params_from_dict(data: dict[str, object]) -> MomentumParams:
        return params_from_dict(data)


def get_strategy() -> CrossSectionalMomentumStrategy:
