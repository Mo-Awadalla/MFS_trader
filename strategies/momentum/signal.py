"""Cross-sectional momentum v1 signal construction.

Canonical hypothesis: recent winners outperform recent losers. Momentum v1
uses the classic 12-1 formation return: cumulative return over months 12
through 2, excluding the most recent month.
"""
...
class MomentumParams:
    """Frozen Momentum v1 hypothesis parameters."""

    formation_lookback_days: int = 252
...
def generate_signals(df: pd.DataFrame, params: MomentumParams) -> pd.DataFrame:
    """Generate Momentum target portfolio weights.

    Input columns must be a MultiIndex of ``(symbol, field)`` where each symbol
    has open/high/low/close/volume columns. For each monthly rebalance date,
    signal formation uses data strictly before that date.
    """
    validate_inputs(df)
    ...
    for ts in df.index:
        ...
        signal = _twelve_minus_one_return(formation, params)
...
def validate_inputs(df: pd.DataFrame) -> None:
    validate_panel_inputs(df, "Momentum")
...
def sweep_grid() -> list[MomentumParams]:
    """No tuning grid for v1; adjacent hypotheses become new Experiments."""
    return [default_params()]


def compact_sweep_grid() -> list[MomentumParams]:
    return [default_params()]


def params_to_dict(params: MomentumParams) -> dict[str, int | float]:
    return {
        "formation_lookback_days": params.formation_lookback_days,
        "skip_recent_days": params.skip_recent_days,
        "liquidity_lookback_days": params.liquidity_lookback_days,
        "min_history_days": params.min_history_days,
        "min_price": params.min_price,
        "min_median_dollar_volume": params.min_median_dollar_volume,
...
def params_from_dict(data: dict[str, Any]) -> MomentumParams:
    return MomentumParams(
        formation_lookback_days=int(data.get("formation_lookback_days", 252)),
        skip_recent_days=int(data.get("skip_recent_days", 21)),
...
def _minimum_lookback(params: MomentumParams) -> int:
    return max(
        params.min_history_days,
        params.liquidity_lookback_days,
        params.formation_lookback_days + 1,
    )


def _twelve_minus_one_return(
    formation: pd.DataFrame,
    params: MomentumParams,
) -> pd.Series:
    close = formation.xs("close", axis=1, level=1)
    end = close.iloc[-(params.skip_recent_days + 1)]
    start = close.iloc[-(params.formation_lookback_days + 1)]
